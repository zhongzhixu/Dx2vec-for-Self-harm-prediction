...
df = df.sample(frac=1, random_state=random_s).reset_index(drop=True) #18,39 good 0.51, 0.68
values = df.values
...
train_X, train_y = train[:, :64*3], train[:, -1]
test_X, test_y = test[:, :64*3], test[:, -1]
train_auxi_X, test_auxi_X = train[:, 64*3:-1], test[:, 64*3:-1]
# reshape input to be 3D [samples, timesteps, features]
train_X = train_X.reshape((train_X.shape[0], 64, int(train_X.shape[1]/64)))
test_X = test_X.reshape((test_X.shape[0], 64, int(test_X.shape[1]/64) ))
print(train_X.shape, train_y.shape, test_X.shape, test_y.shape)
...
THREED_PATH = 'C:/Users/zhongzhi/Documents/Research/HA_project_and_Embedding/full_data/code/three_dim_visualization/'
...
#train_X, train_y = train[:, 64*3:-1], train[:, -1]
#test_X, test_y = test[:, 64*3:-1], test[:, -1]

clf = LogisticRegression(random_state=1, solver='liblinear'  #).fit(trainx, trainy)
                         , class_weight =  {0:.15, 1:.85} ).fit(train_X, train_y) #increase 1's weights to increase 1's recall
...
#====== Single diag===
'''
# must run 3-2 to get dataframe "single_diag" first
test_X = single_diag.values
test_X = test_X[:, 1:-1]
y_pred = clf.predict(test_X)
y_pred_prob = clf.predict_proba(test_X)[:,1] 
rk=pd.DataFrame()
rk['diag'],rk['prob'] = single_diag.iloc[:,0],y_pred_prob
rk.to_csv('single_prob.csv', index = False)
'''
